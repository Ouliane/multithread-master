...
public class MaxValueTest {
    ...
    public void testMax() throws Exception {
        ...
        if (InetAddress.getLocalHost().getHostName().equals("DESKTOP-LS0QEQ6")) {
            System.out.println("\nRunning speedup test");
            assertTrue(parallelTime <= seqTime / ((2. / 3) * numThreads));
            System.out.println(String.format("parallelTime=%s <= seqTime / ((2. / 3) * numThreads)=%s", parallelTime, seqTime / ((2. / 3) * numThreads)));
        }
    }
}
